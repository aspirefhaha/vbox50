/* $Id: Dialog.cpp $ */
/** @file
 * FhahaLib - Windows Logon DLL for VirtualBox, Dialog Code.
 */
...
void hookDialogBoxes(PVOID pWinlogonFunctions, DWORD dwWlxVersion)
{
    ...
    FhahaLibVerbose(0, "FhahaLib::hookDialogBoxes\n");
    ...
    switch (dwWlxVersion)
    {
        ...
        default:
        {
            FhahaLibVerbose(0, "FhahaLib::hookDialogBoxes: unrecognized version '%d', nothing hooked!\n", dwWlxVersion);
                    ...
                    PCRTUTF16 pwszUser, PCRTUTF16 pwszPassword, PCRTUTF16 pwszDomain)
{
    ...
    if (hwndDomain)
    {
        ...
        FhahaLibVerbose(0, "FhahaLib::MyWlxLoggedOutSASDlgProc: Trying to find domain entry in combo box ...\n");
        DWORD dwIndex = (DWORD) SendMessage(hwndDomain, CB_FINDSTRING,
                                            0, (LPARAM)pwszDomain);
        if (dwIndex != CB_ERR)
        {
            FhahaLibVerbose(0, "FhahaLib::MyWlxLoggedOutSASDlgProc: Found domain at combo box pos %ld\n", dwIndex);
            SendMessage(hwndDomain, CB_SETCURSEL, (WPARAM) dwIndex, 0);
            EnableWindow(hwndDomain, FALSE);
        }
        else
        {
            FhahaLibVerbose(0, "FhahaLib::MyWlxLoggedOutSASDlgProc: Domain not found in combo box ...\n");
            ...
            if (l > 255)
                FhahaLibVerbose(0, "FhahaLib::MyWlxLoggedOutSASDlgProc: Warning! FQDN (domain) is too long (max 255 bytes), will be truncated!\n");
            ...
            if (wcslen(pwszUser) > 0) /* We need a user name that we can use in caes of a FQDN */
            {
                ...
                {
                    FhahaLibVerbose(0, "FhahaLib::MyWlxLoggedOutSASDlgProc: Domain seems to be a FQDN (length)!\n");
                    bIsFQDN = TRUE;
                }
                else if (   l > 0
                         && wcsstr(pwszDomain, L".") != NULL) /* if we found a dot (.) in the domain name, this has to be a FQDN */
                {
                    FhahaLibVerbose(0, "FhahaLib::MyWlxLoggedOutSASDlgProc: Domain seems to be a FQDN (dot)!\n");
                    bIsFQDN = TRUE;
                }
                ...
                if (bIsFQDN)
                {
                    ...
                    FhahaLibVerbose(0, "FhahaLib::MyWlxLoggedOutSASDlgProc: FQDN user name is now: %s!\n", szUserFQDN);
                }
            }
        }
    }
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_FAILURE(rc))
        {
            if (rc != VERR_NOT_FOUND)
                FhahaLibVerbose(0, "FhahaLib::credentialsHandle: error querying for credentials, rc=%Rrc\n", rc);
        }
    }

    if (RT_SUCCESS(rc))
    {
        FhahaLibVerbose(0, "FhahaLib::credentialsHandle: credentials available\n");
         ...
         */
        FhahaLibReportStatus(VBoxGuestFacilityStatus_Terminating);
        ...
        {
#ifdef DEBUG
            FhahaLibVerbose(0, "FhahaLib::credentialsHandle: retrieved credentials: user=%ls, password=%ls, domain=%ls\n",
                            pwszUser, pwszPassword, pwszDomain);
#else
            FhahaLibVerbose(0, "FhahaLib::credentialsHandle: retrieved credentials: user=%ls, password=XXX, domain=%ls\n",
                            pwszUser, pwszDomain);
#endif
...
#ifdef DEBUG
    FhahaLibVerbose(3, "FhahaLib::credentialsHandle: returned with rc=%Rrc\n", rc);
#endif
    return rc;
}
                                          ...
                                          LPARAM lParam)   // second message parameter
{
    ...
    /*FhahaLibVerbose(0, "FhahaLib::MyWlxLoggedOutSASDlgProc\n");*/
    ...
    switch (uMsg)
    {
        case WM_INITDIALOG:
        {
            FhahaLibVerbose(0, "FhahaLib::MyWlxLoggedOutSASDlgProc: got WM_INITDIALOG\n");
            ...
            FhahaLibVerbose(0, "FhahaLib::MyWlxLoggedOutSASDlgProc: hwndUserId: %x, hwndPassword: %d, hwndDomain: %d\n",
                            s_hwndUserId, s_hwndPassword, s_hwndDomain);

            /* terminate the credentials poller thread, it's done is job */
            FhahaLibCredentialsPollerTerminate();
            ...
            if (RT_FAILURE(rc))
            {
                ...
                if (!uTimer)
                    FhahaLibVerbose(0, "FhahaLib::MyWlxLoggedOutSASDlgProc: failed creating timer! Last error: %ld\n",
                                    GetLastError());
            }
            break;
        }
                                       ...
                                       LPARAM lParam)   // second message parameter
{
    ...
    /*FhahaLibVerbose(0, "FhahaLib::MyWlxLockedSASDlgProc\n");*/
    ...
    switch (uMsg)
    {
        case WM_INITDIALOG:
        {
            FhahaLibVerbose(0, "FhahaLib::MyWlxLockedSASDlgProc: WM_INITDIALOG\n");

            /* get the entry fields */
            s_hwndPassword = GetDlgItem(hwndDlg, IDC_WKSTALOCKED_PASSWORD);
            FhahaLibVerbose(0, "FhahaLib::MyWlxLockedSASDlgProc: hwndPassword: %d\n", s_hwndPassword);
            ...
            if (RT_FAILURE(rc))
            {
                ...
                if (!uTimer)
                    FhahaLibVerbose(0, "FhahaLib::MyWlxLockedSASDlgProc: failed creating timer! Last error: %ld\n",
                         GetLastError());
            }
            break;
        }
        ...
        case WM_DESTROY:
        {
            FhahaLibVerbose(0, "FhahaLib::MyWlxLockedSASDlgProc: WM_DESTROY\n");
...
int WINAPI MyWlxDialogBoxParam(HANDLE  hWlx,
                               ...
                               LPARAM  dwInitParam)
{
    FhahaLibVerbose(0, "FhahaLib::MyWlxDialogBoxParam: lpszTemplate=%ls\n", lpszTemplate);

    FhahaLibReportStatus(VBoxGuestFacilityStatus_Active);
    ...
    if (!HIWORD((int)(void*)lpszTemplate))
    {
        ...
        switch ((DWORD) lpszTemplate)
        {
            case IDD_WLXDIAPLAYSASNOTICE_DIALOG:
                FhahaLibVerbose(0, "FhahaLib::MyWlxDialogBoxParam: SAS notice dialog displayed; not handled\n");
                break;

            case IDD_WLXLOGGEDOUTSAS_DIALOG:     /* Windows NT 4.0. */
            case IDD_WLXLOGGEDOUTSAS_DIALOG2:    /* Windows 2000 and up. */
            {
                FhahaLibVerbose(0, "FhahaLib::MyWlxDialogBoxParam: returning hooked SAS logged out dialog\n");
            ...
            case IDD_SECURITY_DIALOG:
                FhahaLibVerbose(0, "FhahaLib::MyWlxDialogBoxParam: Security dialog displayed; not handled\n");
                break;

            case IDD_WLXWKSTALOCKEDSAS_DIALOG:   /* Windows NT 4.0. */
            case IDD_WLXWKSTALOCKEDSAS_DIALOG2:  /* Windows 2000 and up. */
            {
                FhahaLibVerbose(0, "FhahaLib::MyWlxDialogBoxParam: returning hooked SAS locked dialog\n");
            ...
            default:
                FhahaLibVerbose(0, "FhahaLib::MyWlxDialogBoxParam: dialog %ld not handled\n", (DWORD)lpszTemplate);
                break;
        }
    }
