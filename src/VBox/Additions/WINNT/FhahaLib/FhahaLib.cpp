...
BOOL WINAPI WlxNegotiate(DWORD dwWinlogonVersion,
                         DWORD *pdwDllVersion)
{
    FhahaLibVerbose(0, "FhahaLib::WlxNegotiate: dwWinlogonVersion: %ld\n", dwWinlogonVersion);
    ...
    if (RT_FAILURE(rc))
    {
        FhahaLibVerbose(0, "FhahaLib::WlxNegotiate: failed loading MSGINA! rc=%Rrc\n", rc);
        return FALSE;
    }
    ...
    if (!GWlxNegotiate)
    {
        FhahaLibVerbose(0, "FhahaLib::WlxNegotiate: failed resolving WlxNegotiate\n");
        return FALSE;
    }
    GWlxInitialize = (PGWLXINITIALIZE)RTLdrGetFunction(hLdrMod, "WlxInitialize");
    if (!GWlxInitialize)
    {
        FhahaLibVerbose(0, "FhahaLib::WlxNegotiate: failed resolving WlxInitialize\n");
        return FALSE;
    }
    ...
    {
        FhahaLibVerbose(0, "FhahaLib::WlxNegotiate: failed resolving WlxDisplaySASNotice\n");
        return FALSE;
    }
    GWlxLoggedOutSAS =
    ...
    if (!GWlxLoggedOutSAS)
    {
        FhahaLibVerbose(0, "FhahaLib::WlxNegotiate: failed resolving WlxLoggedOutSAS\n");
        return FALSE;
    }
    ...
    {
        FhahaLibVerbose(0, "FhahaLib::WlxNegotiate: failed resolving WlxActivateUserShell\n");
        return FALSE;
    }
    GWlxLoggedOnSAS =
    ...
    if (!GWlxLoggedOnSAS)
    {
        FhahaLibVerbose(0, "FhahaLib::WlxNegotiate: failed resolving WlxLoggedOnSAS\n");
        return FALSE;
    }
    ...
    {
        FhahaLibVerbose(0, "FhahaLib::WlxNegotiate: failed resolving WlxDisplayLockedNotice\n");
        return FALSE;
    }
    GWlxIsLockOk = (PGWLXISLOCKOK)RTLdrGetFunction(hLdrMod, "WlxIsLockOk");
    if (!GWlxIsLockOk)
    {
        FhahaLibVerbose(0, "FhahaLib::WlxNegotiate: failed resolving WlxIsLockOk\n");
        return FALSE;
    }
    GWlxWkstaLockedSAS =
    ...
    {
        FhahaLibVerbose(0, "FhahaLib::WlxNegotiate: failed resolving WlxWkstaLockedSAS\n");
        return FALSE;
    }
    GWlxIsLogoffOk = (PGWLXISLOGOFFOK)RTLdrGetFunction(hLdrMod, "WlxIsLogoffOk");
    if (!GWlxIsLogoffOk)
    {
        FhahaLibVerbose(0, "FhahaLib::WlxNegotiate: failed resolving WlxIsLogoffOk\n");
        return FALSE;
    }
    GWlxLogoff = (PGWLXLOGOFF)RTLdrGetFunction(hLdrMod, "WlxLogoff");
    if (!GWlxLogoff)
    {
        FhahaLibVerbose(0, "FhahaLib::WlxNegotiate: failed resolving WlxLogoff\n");
        return FALSE;
    }
    GWlxShutdown = (PGWLXSHUTDOWN)RTLdrGetFunction(hLdrMod, "WlxShutdown");
    if (!GWlxShutdown)
    {
        FhahaLibVerbose(0, "FhahaLib::WlxNegotiate: failed resolving WlxShutdown\n");
        return FALSE;
    }
    /* GINA 1.1, optional */
    ...
    FhahaLibVerbose(0, "FhahaLib::WlxNegotiate: optional function pointers:\n"
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxInitialize\n");
    ...
    FhahaLibReportStatus(VBoxGuestFacilityStatus_Init);
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxDisplaySASNotice\n");

    /* Check if there are credentials for us, if so simulate C-A-D */
    int rc = VbglR3CredentialsQueryAvailability();
    if (RT_SUCCESS(rc))
    {
        FhahaLibVerbose(0, "FhahaLib::WlxDisplaySASNotice: simulating C-A-D\n");
        /* Wutomatic C-A-D */
        pWlxFuncs->WlxSasNotify(hGinaWlx, WLX_SAS_TYPE_CTRL_ALT_DEL);
    }
    else
    {
        FhahaLibVerbose(0, "FhahaLib::WlxDisplaySASNotice: starting credentials poller\n");
        /* start the credentials poller thread */
        FhahaLibCredentialsPollerCreate();
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxLoggedOutSAS\n");
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxActivateUserShell\n");
     ...
     */
    FhahaLibReportStatus(VBoxGuestFacilityStatus_Terminated);
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxLoggedOnSAS: dwSasType=%ld\n", dwSasType);

    /*
     * We don't want to do anything special here since the OS should behave
     * as FhahaLib wouldn't have been installed. So pass all calls down
     * to the original MSGINA.
     */

    /* Forward call to MSGINA. */
    FhahaLibVerbose(0, "FhahaLib::WlxLoggedOnSAS: Forwarding call to MSGINA ...\n");
    if (GWlxLoggedOnSAS)
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxDisplayLockedNotice\n");

    /* Check if there are credentials for us, if so simulate C-A-D */
    int rc = VbglR3CredentialsQueryAvailability();
    if (RT_SUCCESS(rc))
    {
        FhahaLibVerbose(0, "FhahaLib::WlxDisplayLockedNotice: simulating C-A-D\n");
        /* Automatic C-A-D */
        pWlxFuncs->WlxSasNotify(hGinaWlx, WLX_SAS_TYPE_CTRL_ALT_DEL);
    }
    else
    {
        FhahaLibVerbose(0, "FhahaLib::WlxDisplayLockedNotice: starting credentials poller\n");
        /* start the credentials poller thread */
        FhahaLibCredentialsPollerCreate();
        /* Forward call to MSGINA. */
        if (GWlxDisplayLockedNotice)
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxIsLockOk\n");
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxWkstaLockedSAS, dwSasType=%ld\n", dwSasType);
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxIsLogoffOk\n");
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxLogoff\n");

    /* No need to report the "active" status to the host here -- this will be done
     * when FhahaLib gets the chance to hook the dialogs (again). */
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxShutdown\n");
     ...
     */
    FhahaLibReportStatus(VBoxGuestFacilityStatus_Inactive);
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxScreenSaverNotify, pSecure=%d\n",
                    pSecure ? *pSecure : 0);

    /* Report the status to "init" since the screensaver
     * (Winlogon) does not give FhahaLib yet the chance to hook into dialogs
     ...
     *       FhahaLib loaded and set the status to "terminated" again properly
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxStartApplication: pWlxCtx=%p, pszDesktopName=%ls, pEnvironment=%p, pszCmdLine=%ls\n",
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxNetworkProviderLoad\n");
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxDisplayStatusMessage\n");
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxGetStatusMessage\n");
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxRemoveStatusMessage\n");
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxGetConsoleSwitchCredentials\n");
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxReconnectNotify\n");
...
{
    FhahaLibVerbose(0, "FhahaLib::WlxDisconnectNotify\n");
...
{
    FhahaLibVerbose(0, "FhahaLib::WnpScreenSaverStop\n");
